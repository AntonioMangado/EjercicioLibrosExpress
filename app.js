...
app.get("/author/dante-alighieri", (req, res) => {
    const book = books.find(element => element.author === "Dante Alighieri")
    res.status(200).json(book.title);
})

// Obtener pais de libro de Charles Dickens
app.get("/country/charles-dickens", (req, res) => {
    const book = books.find(element => element.author === "Charles Dickens")
    res.status(200).json(book.country);
})

// Obtener paginas y año de Cervantes
app.get("/year&pages/cervantes", (req, res) => {
    const book = books.find(element => element.author === "Miguel de Cervantes")
